 ...
 */
package com.example.kiantablet.records;
...
public class UriRecord implements ParsedNdefRecord {
    ...
    public static byte getPrefixByte(String value) {
    	
          if (URI_PREFIX_MAP.containsValue(value))
          {
              for (BiMap.Entry<Byte, String> e : URI_PREFIX_MAP.entrySet()) {
            	  byte key = e.getKey();
    ...
    public View getParsedView(Activity activity)  {
    ...
    private static UriRecord parseWellKnown(NdefRecord record) {
        ...
        byte[] fullUri =
            Bytes.concat(prefix.getBytes(Charset.forName("UTF-8")), Arrays.copyOfRange(payload, 1,payload.length));
    ...
    public static boolean isUri(NdefRecord record) {
        try {
        	...
        	Log.e(Class.class.getClass().getSimpleName(), "parse", e);
            return false;
        }
    }
